...
window.onload = function () {
  ...
  favorites.forEach(function (favoriteHTML) {
    ...
    // Append the remove button to the favorite item
...
function checkLoyalty() {
  ...
  // Display loyalty points in the custom modal
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  checkInDateField.setAttribute('min', currentDate); // Set the minimum date for check-in as the current date
  ...
  checkInDateField.addEventListener('change', function() {
    ...
    nextDay.setDate(nextDay.getDate() + 1); // Set the check-out minimum date to be one day after check-in

    const minCheckOutDate = nextDay.toISOString().split('T')[0]; // Format the minimum date in YYYY-MM-DD
